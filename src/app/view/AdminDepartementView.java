...
public class AdminDepartementView extends BorderPane {
    ...
    private TableColumn<Departement, Integer> colIdUfr;
    private TableColumn<Departement, String> colNomUfr;
    ...
    private void initLeftTableSection() {
        ...
        Label lblTitle = new Label("Gestion des Départements");
        ...
        tableDepartement = new TableView<>();
        tableDepartement.setPlaceholder(new Label("Aucun contenu dans la table"));
        tableDepartement.setColumnResizePolicy(TableView.CONSTRAINED_RESIZE_POLICY_FLEX_LAST_COLUMN);
        tableDepartement.setPrefHeight(550);
        ...
        colIdUfr = new TableColumn<>("Id UFR");
        colIdUfr.setCellValueFactory(new PropertyValueFactory<>("ufr_id"));

        colNomUfr = new TableColumn<>("Nom UFR");
        colNomUfr.setCellValueFactory(new PropertyValueFactory<>("ufrNom"));

        tableDepartement.getColumns().add(colId);
        tableDepartement.getColumns().add(colNom);
        tableDepartement.getColumns().add(colIdUfr);
        tableDepartement.getColumns().add(colNomUfr);

        leftBox.getChildren().addAll(headerBox, tableDepartement);
    ...
    private void initRightFormSection() {
        ...
        Label lblFormTitle = new Label("Informations Département");
        ...
        txtNom.setPromptText("Ex: Informatique");
        VBox fieldNom = createFormField("Nom :", txtNom);

        txtUfrId = new TextField();
        txtUfrId.setPromptText("Ex: 1");
        txtUfrId.textProperty().addListener((observable, oldValue, newValue) -> {
            if (!newValue.matches("\\d*")) {
                txtUfrId.setText(newValue.replaceAll("[^\\d]", ""));
            }
        });
        VBox fieldUfrId = createFormField("Id UFR :", txtUfrId);
    ...
    public TableView<Departement> getTableDepartement() { return tableDepartement; }
    ...
    public TextField getTxtUfrId() { return txtUfrId; }
